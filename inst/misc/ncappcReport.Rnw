...
require(xtable)
print(xtable(pddf, align=rep("c",ncol(pddf)+1), rotate.colnames = T), file=paste(usrdir,"/sum.tex",sep=""), include.rownames=FALSE,
...
<<child = if (nsim == 0) 'figTabNoSim.Rnw', echo=FALSE>>=
@

<<child = if (nsim != 0) 'figTab.Rnw', echo=FALSE>>=
@
