...
require(xtable)
mtable <- xtable(pddf, align=c(rep("c",ncol(pddf)-3),"p{3cm}","p{1cm}","p{1cm}","p{2cm}"), rotate.colnames = T)
...
<<concplot, echo=FALSE, fig.height=5, fig.width=6, out.width='1\\textwidth', fig.align="center">>=
if (length(conc)>0){
...
<<histobsplot, echo=FALSE, fig.height=12, fig.width=10, fig.pos="H", out.width='1\\textwidth', fig.align='center'>>=
if (length(histobs)>0){
...
Histogram of four selected NCA metrics (AUClast, AUCINF\_obs, Cmax, Tmax) estimated from the observed data. The solid blue vertical and dotted lines represent the population mean and the "spread" of the data. The "spread" is defined by \Sexpr{print(sprtxt)} of the NCA metrics obtained from the study population.

\newpage
\subsection{Figure 3. [Population level] Histogram of the simulated population means of the NCA metrics}
<<popplot, echo=FALSE, fig.height=hth, fig.width=wth, fig.pos="H", out.width='1\\textwidth', fig.align='center'>>=
if (length(pop)>0){
  suppressPackageStartupMessages(require(gridExtra))
...
Histogram of the population mean of the NCA metrics obtained from the simulated data from the \Sexpr{round(nsim)} simulations. The red and blue solid vertical lines represent the population mean of the NCA metric obtained from the observed data and the mean of the population means of the same NCA metric obtained from the \Sexpr{round(nsim)} number of simulations, respectively. The blue dashed vertical lines represent the "spread" of the simulated distribution. The "spread" is defined by \Sexpr{print(sprtxt)} of the NCA metrics obtained from the study population.

\newpage
\subsection{Figure 4. [Individual level] Deviation of the simulated NCA metrics from the observed value}
<<devplot, echo=FALSE, fig.pos="H", fig.height=hth, fig.width=wth, out.width='1\\textwidth', fig.align='center'>>=
if (length(dev)>0){
...
\newpage
\subsection{Figure 7. [Individual level] NPDE values of the NCA metrics for each individual}
<<npdeplot, echo=FALSE, fig.pos="H", out.width='1\\textwidth', fig.align='center'>>=
if (length(npde)>0){
...
\newpage
\subsection{Figure 8. [Population level] Histogram of the NPDE values of the NCA metrics within a given population group}
<<histnpdeplot, echo=FALSE, fig.pos="H", out.width='1\\textwidth', fig.align='center'>>=
