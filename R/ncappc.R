...
ncappc <- function(obsFile=NULL,simFile=NULL,grNm=NULL,grp=NULL,flNm=NULL,flag=NULL,doseNm=NULL,dose=NULL,concUnit=NULL,timeUnit=NULL,doseUnit=NULL,doseNormUnit=NULL,obsLog="FALSE",simLog="FALSE",psnOut="FALSE",idNmObs="ID",timeNmObs="TIME",concNmObs="DV",idNmSim="ID",timeNmSim="TIME",concNmSim="DV",AUCTimeRange=NULL,backExtrp="FALSE",LambdaTimeRange=NULL,LambdaExclude=NULL,adminType="extravascular",doseType="ns",Tau=NULL,TI=NULL,doseAmtNm=NULL,doseAmt=NULL,method="linear",blqNm=NULL,blqExcl=1,evid="FALSE",evidIncl=0,mdv="FALSE",filterNm=NULL,filterExcl=NULL,negConcExcl="FALSE",param=c("AUClast","Cmax"),timeFormat="number",dateColNm=NULL,dateFormat=NULL,spread="pi",tabCol=c("AUClast","Cmax","Tmax","AUCINF_obs","Vz_obs","Cl_obs","HL_Lambda_z"),printOut="TRUE",figFormat="tiff"){
  ...
  # Install required packages and the function to calculate the parameters
  pckgNm <- c("ggplot2","grid","gridExtra","scales","gtable","PerformanceAnalytics","lattice","knitr","xtable","reshape2")
  for (p in 1:length(pckgNm)){
    if (pckgNm[p]%in%row.names(installed.packages())==F){install.packages(repos='http://cran.us.r-project.org', pckgNm[p])}
  }
  ...
  suppressWarnings(suppressPackageStartupMessages(require(gtable)))
  suppressWarnings(suppressPackageStartupMessages(require(lattice)))
  suppressWarnings(suppressPackageStartupMessages(require(PerformanceAnalytics)))
  suppressWarnings(suppressPackageStartupMessages(require(knitr)))
  suppressWarnings(suppressPackageStartupMessages(require(xtable)))
  ...
  if (case == 1){
    ...
    for (d in 1:ndose){
      ...
      for (i in 1:length(idd)){
        ...
        NCAprm <- est.nca(time,conc,backExtrp,negConcExcl,doseType,adminType,doseAmount,method,AUCTimeRange,LambdaTimeRange,LambdaExclude,Tau,TI,simFile,dset)  # calls est.nca function
    ...
    for (p in seq(1,nfct,3)){
      ...
      if (printOut=="TRUE") ggsave(file=paste(usrdir,"/TimeConc_",str,".",figFormat,sep=""),plot=gdr,height=dht,width=dwd,units="cm",dpi=600)
    }
  }
  if (case == 2){
      ...
      for (d in 1:ndose){
        ...
        for (i in 1:length(stNm)){
          ...
          if (length(tdf) < 2){
            ...
            nm <- rbind(nm, data.frame(Nm))
            stPrm <- calc.stat(arr=tdf)        # calls calc.stat function
            stPrm <- unname(stPrm)
            pm[counter,] <- stPrm
          }
          counter <- counter + 1
  ...
  if (is.null(simFile)){
    ...
    lasdf <- lapply(list.files(pattern="sim_[0-9]*.csv",full=T),function(i){read.csv(i, header=T)})
    ...
    if (case == 1){
      ...
      for (d in 1:ndose){
        ...
        if (!is.null(ggdev)){
          ...
          if (printOut=="TRUE") suppressMessages(suppressWarnings(ggsave(file=paste(usrdir,"/Deviation_",figlbl,".",figFormat,sep=""),height=hth,width=wth,units="cm",dpi=600)))
        ...
        if (printOut=="TRUE") suppressMessages(suppressWarnings(ggsave(file=paste(usrdir,"/NPDE_",figlbl,".",figFormat,sep=""),height=hth,width=wth,units="cm",dpi=600)))
        
        histnpdeplot[[length(histnpdeplot)+1]] <- npdeout$gghnpde
        suppressMessages(suppressWarnings(print(npdeout$gghnpde)))
        if (printOut=="TRUE") suppressMessages(suppressWarnings(ggsave(file=paste(usrdir,"/histNPDE_",figlbl,".",figFormat,sep=""),height=hth,width=wth,units="cm",dpi=600)))
      }
      ...
      if (printOut=="TRUE") suppressMessages(suppressWarnings(ggsave(file=paste(usrdir,"/forestNPDE.",figFormat,sep=""),height=hth,width=wth,units="cm",dpi=600)))
    }else if (case==2){
      for (g in 1:ngrp){
        for (d in 1:ndose){
