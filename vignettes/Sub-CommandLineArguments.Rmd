...
grNm | Column name for population stratifier (1st level stratifier if used with flNm) | Column name | **NULL**
grp | Population stratification ID | c(1,2) | **NULL**
flNm | Column name for popualtion stratifier (2nd level stratifier if used with grNm) | Column name | **NULL**
flag | Population stratification ID | c(1,2) | **NULL**
doseNm | Column name to specify dose identifiers | **NULL**
dose | Dose identifiers to be used | c(1,2) | **NULL**
...
**grNm, flNm and doseNm**
All three of them individually can be used to define population group when used separately. E.g., if you have only one stratification criteria for the population, any of these 3 arguments can be used. When doseNm is used together with either grNm or flNm, grNm/flNm defines the upper level stratification and doseNm defines the lower level stratification. When all three of them used together, grNm, flNm and doseNm define the upper, middle and lower stratification levels, respectively. The default value for these arguments are ***NULL***.

**grp, flag and dose**
"grp", "flag" and "dose" are identfiers for the population stratifications, present in the corresponding stratification levels defined by "grNm", "flNm" or "doseNm" columns, respectively. The default value for this argument is ***NULL***. If the "grNm", "flNm" or "doseNm" arguments are non-empty, but the "grp" and "flag" arguments are left as NULL, this function will use the unique values from the "grNm", "flNm" or "doseNm" column as identifiers for the population strata.
